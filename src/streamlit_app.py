...
def load_pipeline_or_parts(
    pipeline_path: Path, model_path: Path, vectorizer_path: Path
):
...
def main():
    ...
    st.set_page_config(
        page_title="Fake News Detector", page_icon="📰", layout="centered"
    )
    ...
    with st.sidebar:
        ...
        st.write(
            f"Exists → pipeline: **{pipeline_path.exists()}**, "
